...
export default function NewDogPage() {
  ...
  const [form, setForm] = useState({
    ...
    guardian_name: '', guardian_contact: '', current_treatment: '',
  })

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setLoading(true)
    const { error } = await supabase.from('dogs').insert([{ ...form, status: 'Active' }])
    setLoading(false)
  ...
  return (
    <div className="pb-nav">
      ...
      <form onSubmit={handleSubmit} className="px-4 space-y-4">
        ...
        <Field label="Current Ongoing Treatment">
          <textarea value={form.current_treatment} onChange={e => setForm(f => ({ ...f, current_treatment: e.target.value }))} placeholder="Describe the current treatment plan..." rows={3} className={inputCls} />
        </Field>
        <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white py-3 rounded-2xl font-semibold text-sm disabled:opacity-60 mt-2">
